...
class Trade:
    ...
    def __init__(
        ...
        Args:
            ...
            asset: Asset symbol (SPY, TLT, GLD)
        """
...
class Backtester:
    ...
    def open_trade(
        ...
        logger.debug(
            f"OPEN {direction} {asset}: {quantity:.2f} @ ${entry_price:.2f} "
    ...
    def run(
        ...
        safe_haven_asset: str = 'none'
    ) -> pd.DataFrame:
        """
        Run backtest on historical data with multi-asset support.

        Args:
            signals_df: DataFrame with signals (from SignalEngine)
            asset_data: Dict of asset DataFrames {'SPY': df, 'TLT': df, 'GLD': df}
            safe_haven_asset: Asset to trade on RISK-OFF ('none', 'TLT', 'GLD')
        ...
        for timestamp in common_index:
            ...
            # Use the price data for the asset we're currently trading
            if self.current_trade is not None:
                current_asset_data = asset_data[self.current_trade.asset].loc[timestamp]
                current_price = current_asset_data['Close']
                high = current_asset_data['High']
                low = current_asset_data['Low']
            ...
            if self.current_trade is None:
                if consensus == 'RISK-ON' and spy_rsi > 50:
                    # LONG SPY on RISK-ON
                    spy_price = asset_data['SPY'].loc[timestamp]['Close']
                    self.open_trade(timestamp, spy_price, 'LONG', asset='SPY', momentum_confirmed=True)

                elif consensus == 'RISK-OFF' and spy_rsi < 50:
                    # LONG safe haven on RISK-OFF (if configured)
                    if safe_haven_asset != 'none' and safe_haven_asset in asset_data:
                        safe_haven_price = asset_data[safe_haven_asset].loc[timestamp]['Close']
                        self.open_trade(timestamp, safe_haven_price, 'LONG', asset=safe_haven_asset, momentum_confirmed=True)
                    # else: sit flat (original behavior)

            # Exit on signal reversal
            elif self.current_trade is not None:
                # Close position if signal changes
                current_asset = self.current_trade.asset
                current_asset_price = asset_data[current_asset].loc[timestamp]['Close']

                # If holding SPY and signal flips to RISK-OFF
                if current_asset == 'SPY' and consensus == 'RISK-OFF':
                    self.close_trade(timestamp, current_asset_price, 'SIGNAL_REVERSAL')

                # If holding safe haven and signal flips to RISK-ON
                elif current_asset in ['TLT', 'GLD'] and consensus == 'RISK-ON':
                    self.close_trade(timestamp, current_asset_price, 'SIGNAL_REVERSAL')

            # Record equity
            current_equity = self.current_capital
            if self.current_trade is not None:
                # Calculate unrealized P&L using current asset's price
                current_asset_price = asset_data[self.current_trade.asset].loc[timestamp]['Close']

                if self.current_trade.direction == 'LONG':
                    unrealized_pnl = (current_asset_price - self.current_trade.entry_price) * self.current_trade.quantity
                else:
                    unrealized_pnl = (self.current_trade.entry_price - current_asset_price) * self.current_trade.quantity
            ...
            self.equity_curve.append({
                ...
                'asset': self.current_trade.asset if self.current_trade else None
            })
        ...
        if self.current_trade is not None:
            final_asset = self.current_trade.asset
            final_price = asset_data[final_asset]['Close'].iloc[-1]
